...
public:
  ...
  ///
  /// The numeric values are exactly the values Apple's compiler exposes as the
  /// predefined macro `__METAL_VERSION__` (measured in
  /// reference/metal-ast-*/meta/metal-predefined-macros.txt, e.g. MSL 4.0 =>
  ...
  /// Selects which set of single precision math functions the standard library
  /// should route to. Mirrors `-fmetal-math-fp32-functions=`.
  enum class MetalFPMathFunctions {
    Fast,    ///< metal::fast namespace (Apple's default)
    Precise, ///< metal::precise namespace
  };
