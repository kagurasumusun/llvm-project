//===- AIRBitcodeWriter.cpp - AIR Bitcode Writer (typed pointers) --------===//
//
...
#include "AIRBitcodeWriter.h"  // internal header
...
public:
  ...
  AIRBitcodeWriter(const Module &M, SmallVectorImpl<char> &Buffer,
      ...
      : I8Ty(Type::getInt8Ty(M.getContext())),
        ...
        PointerMap(air::computePointerTypeMap(M)) {
...
private:
  ...
  void writeGenericDINode(const GenericDINode *N,
    ...
    llvm_unreachable("AIR cannot contain GenericDI Nodes");
  }
  ...
  void writeDIGenericSubrange(const DIGenericSubrange *N,
                              SmallVectorImpl<uint64_t> &Record,
                              unsigned Abbrev) {
    llvm_unreachable("AIR cannot contain DIGenericSubrange Nodes");
  }
  ...
  void writeDIFixedPointType(const DIFixedPointType *N,
                             SmallVectorImpl<uint64_t> &Record,
                             unsigned Abbrev) {
    llvm_unreachable("AIR cannot contain DIFixedPointType Nodes");
  }
  void writeDIStringType(const DIStringType *N,
                         SmallVectorImpl<uint64_t> &Record, unsigned Abbrev) {
    llvm_unreachable("AIR cannot contain DIStringType Nodes");
  }
  void writeDIDerivedType(const DIDerivedType *N,
                          SmallVectorImpl<uint64_t> &Record, unsigned Abbrev);
  void writeDISubrangeType(const DISubrangeType *N,
                           SmallVectorImpl<uint64_t> &Record, unsigned Abbrev) {
    llvm_unreachable("AIR cannot contain DISubrangeType Nodes");
  }
  ...
  void writeDICommonBlock(const DICommonBlock *N,
                          SmallVectorImpl<uint64_t> &Record, unsigned Abbrev) {
    llvm_unreachable("AIR cannot contain DICommonBlock Nodes");
  }
  ...
  void writeDIMacro(const DIMacro *N, SmallVectorImpl<uint64_t> &Record,
                    unsigned Abbrev) {
    llvm_unreachable("AIR cannot contain DIMacro Nodes");
  }
  void writeDIMacroFile(const DIMacroFile *N, SmallVectorImpl<uint64_t> &Record,
                        unsigned Abbrev) {
    llvm_unreachable("AIR cannot contain DIMacroFile Nodes");
  }
  void writeDIArgList(const DIArgList *N, SmallVectorImpl<uint64_t> &Record,
                      unsigned Abbrev) {
    llvm_unreachable("AIR cannot contain DIArgList Nodes");
  }
  void writeDIAssignID(const DIAssignID *N, SmallVectorImpl<uint64_t> &Record,
    ...
    // FIXME: translate DIAssignID to debug info AIR supports.
    //   See https://github.com/llvm/llvm-project/issues/58989
    llvm_unreachable("AIR cannot contain DIAssignID Nodes");
  }
  ...
  void writeDILabel(const DILabel *N, SmallVectorImpl<uint64_t> &Record,
                    unsigned Abbrev) {
    llvm_unreachable("AIR cannot contain DILabel Nodes");
  }
  ...
  void writeDIGlobalVariableExpression(const DIGlobalVariableExpression *N,
                                       SmallVectorImpl<uint64_t> &Record,
                                       unsigned Abbrev) {
    llvm_unreachable("AIR cannot contain GlobalVariableExpression Nodes");
  }
  ...
  ///
  /// GlobalObject types are saved by AIRPointerTypeAnalysis as pointers to the
...
////////////////////////////////////////////////////////////////////////////////
/// Begin air::BitcodeWriter Implementation
////////////////////////////////////////////////////////////////////////////////

air::BitcodeWriter::BitcodeWriter(SmallVectorImpl<char> &Buffer)
...
air::BitcodeWriter::~BitcodeWriter() { }

/// Write the specified module to the specified output stream.
void air::WriteAIRToFile(const Module &M, raw_ostream &Out) {
...
void air::WriteAIRBitcodeToFile(const Module &M, raw_ostream &Out) {
  WriteAIRToFile(M, Out);
}
...
void BitcodeWriter::writeModule(const Module &M) {
  ...
  AIRBitcodeWriter ModuleWriter(M, Buffer, StrtabBuilder, *Stream);
  ModuleWriter.write();
}

////////////////////////////////////////////////////////////////////////////////
/// Begin air::BitcodeWriterBase Implementation
////////////////////////////////////////////////////////////////////////////////

unsigned AIRBitcodeWriter::getEncodedCastOpcode(unsigned Opcode) {
  switch (Opcode) {
  default:
...
unsigned AIRBitcodeWriter::getEncodedUnaryOpcode(unsigned Opcode) {
  switch (Opcode) {
  default:
...
unsigned AIRBitcodeWriter::getEncodedBinaryOpcode(unsigned Opcode) {
  switch (Opcode) {
  default:
    llvm_unreachable("Unknown binary instruction!");
...
unsigned AIRBitcodeWriter::getTypeID(Type *T, const Value *V) {
  if (!T->isPointerTy() &&
...
unsigned AIRBitcodeWriter::getGlobalObjectValueTypeID(Type *T,
...
unsigned AIRBitcodeWriter::getEncodedRMWOperation(AtomicRMWInst::BinOp Op) {
  switch (Op) {
  default:
...
unsigned AIRBitcodeWriter::getEncodedOrdering(AtomicOrdering Ordering) {
  switch (Ordering) {
...
void AIRBitcodeWriter::writeStringRecord(BitstreamWriter &Stream,
...
uint64_t AIRBitcodeWriter::getAttrKindEncoding(Attribute::AttrKind Kind) {
  switch (Kind) {
  ...
  default:
    llvm_unreachable("Trying to encode attribute not supported by AIR. These "
                     "should be stripped in AIRPrepare");
  }

  llvm_unreachable("Trying to encode unknown attribute");
}

void AIRBitcodeWriter::emitSignedInt64(SmallVectorImpl<uint64_t> &Vals,
                                        uint64_t V) {
  if ((int64_t)V >= 0)
    Vals.push_back(V << 1);
  else
...
void AIRBitcodeWriter::emitWideAPInt(SmallVectorImpl<uint64_t> &Vals,
                                      const APInt &A) {
...
uint64_t AIRBitcodeWriter::getOptimizationFlags(const Value *V) {
  uint64_t Flags = 0;
...
unsigned
AIRBitcodeWriter::getEncodedLinkage(const GlobalValue::LinkageTypes Linkage) {
  switch (Linkage) {
...
unsigned AIRBitcodeWriter::getEncodedLinkage(const GlobalValue &GV) {
  return getEncodedLinkage(GV.getLinkage());
}

unsigned AIRBitcodeWriter::getEncodedVisibility(const GlobalValue &GV) {
...
unsigned AIRBitcodeWriter::getEncodedDLLStorageClass(const GlobalValue &GV) {
...
unsigned AIRBitcodeWriter::getEncodedThreadLocalMode(const GlobalValue &GV) {
...
unsigned AIRBitcodeWriter::getEncodedComdatSelectionKind(const Comdat &C) {
...
////////////////////////////////////////////////////////////////////////////////
/// Begin AIRBitcodeWriter Implementation
////////////////////////////////////////////////////////////////////////////////

void AIRBitcodeWriter::writeAttributeGroupTable() {
    ...
    for (Attribute Attr : AS) {
      if (Attr.isEnumAttribute()) {
        ...
        assert(Val <= bitc::ATTR_KIND_ARGMEMONLY &&
               "AIR does not support attributes above ATTR_KIND_ARGMEMONLY");
        Record.push_back(0);
...
void AIRBitcodeWriter::writeAttributeTable() {
...
void AIRBitcodeWriter::writeTypeTable() {
  ...
  for (Type *T : TypeList) {
    ...
    case Type::PointerTyID: {
      ...
      writeStringRecord(Stream, bitc::TYPE_CODE_STRUCT_NAME,
                        "airOpaquePtrReservedName", StructNameAbbrev);
      break;
    }
...
void AIRBitcodeWriter::writeComdats() {
...
void AIRBitcodeWriter::writeValueSymbolTableForwardDecl() {}

/// Emit top-level description of module, including target triple, inline asm,
/// descriptors for global variables, and function prototype info.
/// Returns the bit offset to backpatch with the location of the real VST.
void AIRBitcodeWriter::writeModuleInfo() {
  // Emit various pieces of data attached to a module.

  // Emit the module's actual target triple and data layout.  For AIR targets
  // these will be e.g. "air64_v22-apple-macosx10.15" and the matching
  // data layout.  Using the module values ensures we preserve the exact
  // version suffix that Metal's runtime checks.
  writeStringRecord(Stream, bitc::MODULE_CODE_TRIPLE,
                    M.getTargetTriple(), 0);
  writeStringRecord(Stream, bitc::MODULE_CODE_DATALAYOUT,
                    M.getDataLayout().getStringRepresentation(), 0);
...
void AIRBitcodeWriter::writeValueAsMetadata(
...
void AIRBitcodeWriter::writeMDTuple(const MDTuple *N,
                                     SmallVectorImpl<uint64_t> &Record,
                                     unsigned Abbrev) {
...
void AIRBitcodeWriter::writeDILocation(const DILocation *N,
                                        SmallVectorImpl<uint64_t> &Record,
                                        unsigned &Abbrev) {
  if (!Abbrev)
...
void AIRBitcodeWriter::writeDISubrange(const DISubrange *N,
                                        SmallVectorImpl<uint64_t> &Record,
                                        unsigned Abbrev) {
  Record.push_back(N->isDistinct());
...
void AIRBitcodeWriter::writeDIEnumerator(const DIEnumerator *N,
                                          SmallVectorImpl<uint64_t> &Record,
                                          unsigned Abbrev) {
  Record.push_back(N->isDistinct());
...
void AIRBitcodeWriter::writeDIBasicType(const DIBasicType *N,
                                         SmallVectorImpl<uint64_t> &Record,
                                         unsigned Abbrev) {
  Record.push_back(N->isDistinct());
...
void AIRBitcodeWriter::writeDIDerivedType(const DIDerivedType *N,
                                           SmallVectorImpl<uint64_t> &Record,
                                           unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getTag());
...
void AIRBitcodeWriter::writeDICompositeType(const DICompositeType *N,
                                             SmallVectorImpl<uint64_t> &Record,
                                             unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getTag());
...
void AIRBitcodeWriter::writeDISubroutineType(const DISubroutineType *N,
                                              SmallVectorImpl<uint64_t> &Record,
                                              unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getFlags());
...
void AIRBitcodeWriter::writeDIFile(const DIFile *N,
                                    SmallVectorImpl<uint64_t> &Record,
                                    unsigned Abbrev) {
  Record.push_back(N->isDistinct());
...
void AIRBitcodeWriter::writeDICompileUnit(const DICompileUnit *N,
                                           SmallVectorImpl<uint64_t> &Record,
                                           unsigned Abbrev) {
  Record.push_back(N->isDistinct());
...
void AIRBitcodeWriter::writeDISubprogram(const DISubprogram *N,
                                          SmallVectorImpl<uint64_t> &Record,
                                          unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(VE.getMetadataOrNullID(N->getScope()));
...
void AIRBitcodeWriter::writeDILexicalBlock(const DILexicalBlock *N,
                                            SmallVectorImpl<uint64_t> &Record,
                                            unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(VE.getMetadataOrNullID(N->getScope()));
...
void AIRBitcodeWriter::writeDILexicalBlockFile(
...
void AIRBitcodeWriter::writeDINamespace(const DINamespace *N,
                                         SmallVectorImpl<uint64_t> &Record,
                                         unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(VE.getMetadataOrNullID(N->getScope()));
...
void AIRBitcodeWriter::writeDIModule(const DIModule *N,
                                      SmallVectorImpl<uint64_t> &Record,
                                      unsigned Abbrev) {
  Record.push_back(N->isDistinct());
...
void AIRBitcodeWriter::writeDITemplateTypeParameter(
...
void AIRBitcodeWriter::writeDITemplateValueParameter(
...
void AIRBitcodeWriter::writeDIGlobalVariable(const DIGlobalVariable *N,
                                              SmallVectorImpl<uint64_t> &Record,
                                              unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(VE.getMetadataOrNullID(N->getScope()));
...
void AIRBitcodeWriter::writeDILocalVariable(const DILocalVariable *N,
                                             SmallVectorImpl<uint64_t> &Record,
                                             unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getTag());
...
void AIRBitcodeWriter::writeDIExpression(const DIExpression *N,
                                          SmallVectorImpl<uint64_t> &Record,
                                          unsigned Abbrev) {
...
void AIRBitcodeWriter::writeDIObjCProperty(const DIObjCProperty *N,
                                            SmallVectorImpl<uint64_t> &Record,
                                            unsigned Abbrev) {
  llvm_unreachable("AIR does not support objc!!!");
}

void AIRBitcodeWriter::writeDIImportedEntity(const DIImportedEntity *N,
                                              SmallVectorImpl<uint64_t> &Record,
                                              unsigned Abbrev) {
  Record.push_back(N->isDistinct());
  Record.push_back(N->getTag());
...
unsigned AIRBitcodeWriter::createDILocationAbbrev() {
...
unsigned AIRBitcodeWriter::createGenericDINodeAbbrev() {
...
void AIRBitcodeWriter::writeMetadataRecords(ArrayRef<const Metadata *> MDs,
                                             SmallVectorImpl<uint64_t> &Record,
...
unsigned AIRBitcodeWriter::createMetadataStringsAbbrev() {
  auto Abbv = std::make_shared<BitCodeAbbrev>();
...
void AIRBitcodeWriter::writeMetadataStrings(
...
void AIRBitcodeWriter::writeModuleMetadata() {
...
void AIRBitcodeWriter::writeFunctionMetadata(const Function &F) {
  if (!VE.hasMDs())
    return;
...
void AIRBitcodeWriter::writeFunctionMetadataAttachment(const Function &F) {
...
void AIRBitcodeWriter::writeModuleMetadataKinds() {
  SmallVector<uint64_t, 64> Record;
...
void AIRBitcodeWriter::writeConstants(unsigned FirstVal, unsigned LastVal,
                                       bool isGlobal) {
  if (FirstVal == LastVal)
    return;
...
void AIRBitcodeWriter::writeModuleConstants() {
  const ValueEnumerator::ValueList &Vals = VE.getValues();
...
bool AIRBitcodeWriter::pushValueAndType(const Value *V, unsigned InstID,
                                         SmallVectorImpl<unsigned> &Vals) {
...
void AIRBitcodeWriter::pushValue(const Value *V, unsigned InstID,
                                  SmallVectorImpl<unsigned> &Vals) {
  unsigned ValID = VE.getValueID(V);
  Vals.push_back(InstID - ValID);
}

void AIRBitcodeWriter::pushValueSigned(const Value *V, unsigned InstID,
                                        SmallVectorImpl<uint64_t> &Vals) {
  unsigned ValID = VE.getValueID(V);
...
void AIRBitcodeWriter::writeInstruction(const Instruction &I, unsigned InstID,
                                         SmallVectorImpl<unsigned> &Vals) {
  unsigned Code = 0;
  unsigned AbbrevToUse = 0;
...
void AIRBitcodeWriter::writeFunctionLevelValueSymbolTable(
...
void AIRBitcodeWriter::writeFunction(const Function &F) {
...
void AIRBitcodeWriter::writeBlockInfo() {
...
void AIRBitcodeWriter::write() {
  ...
  // so that it is clear the AIR module version is different.
  AIRBitcodeWriter::writeModuleVersion();
  ...
  // Emit names for globals/functions etc.
  // AIR uses the same format for module-level value symbol table as for the
