...
// Supports Mach-O FAT Universal Wrapper Containers (0xcbfebabe / 0xcafebabe).
...
static const uint32_t FAT_MAGIC_32 = 0xcafebabe;
static const uint32_t FAT_MAGIC_64 = 0xcbfebabe;
...
private:
    ...
    void transform(const uint8_t *data) {
        ...
        for (int i = 0; i < 64; ++i) {
            uint32_t ch = (e & f) ^ (~e & g);
            uint32_t temp1 = h + (rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25)) + ch + k[i] + m[i];
            uint32_t maj = (a & b) ^ (a & c) ^ (b & c);
            uint32_t temp2 = (rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22)) + maj;
...
public:
    ...
    static bool parseMTLBBlock(const uint8_t *data, size_t block_size) {
        if (block_size < 64 || std::memcmp(data, MTLB_MAGIC, 4) != 0) {
            std::cerr << "  Error: Invalid MTLB block magic.\n";
            return false;
        }

        uint32_t ver = 0, total_sz = 0, dir_off = 0, dir_sz = 0, tab_off = 0, tab_sz = 0, bc_off = 0;
        std::memcpy(&ver, data + 4, 4);
        std::memcpy(&total_sz, data + 16, 4);
        std::memcpy(&dir_off, data + 24, 4);
        std::memcpy(&dir_sz, data + 28, 4);
        std::memcpy(&tab_off, data + 40, 4);
        std::memcpy(&tab_sz, data + 44, 4);
        std::memcpy(&bc_off, data + 56, 4);
        ...
        while (idx + 4 <= end_idx && idx < block_size) {
            char tag[5] = {0};
            std::memcpy(tag, data + idx, 4);
            if (std::strcmp(tag, "NAME") == 0) {
                uint16_t nlen = 0;
                std::memcpy(&nlen, data + idx + 4, 2);
                if (idx + 6 + nlen > block_size) break;
                std::string fname(reinterpret_cast<const char*>(data + idx + 6), nlen);
                std::cout << "    [NAME @ 0x" << std::hex << idx << std::dec << "] Function Name: '" << fname << "'\n";
                idx += 6 + nlen;
            } else if (std::strcmp(tag, "HASH") == 0) {
                uint16_t hlen = 0;
                std::memcpy(&hlen, data + idx + 4, 2);
                ...
                uint16_t mlen = 0;
                std::memcpy(&mlen, data + idx + 4, 2);
                uint64_t mdsz = 0;
                if (mlen == 8 && idx + 6 + 8 <= block_size) std::memcpy(&mdsz, data + idx + 6, 8);
                std::cout << "    [MDSZ @ 0x" << std::hex << idx << std::dec << "] Metadata/Bitcode Size Info: " << mdsz << " bytes\n";
                idx += 6 + mlen;
            } else if (std::strcmp(tag, "OFFT") == 0) {
                uint16_t olen = 0;
                std::memcpy(&olen, data + idx + 4, 2);
                if (olen == 24 && idx + 6 + 24 <= block_size) {
                    uint64_t bc_o = 0, bc_s = 0, info_o = 0;
                    std::memcpy(&bc_o, data + idx + 6, 8);
                    std::memcpy(&bc_s, data + idx + 14, 8);
                    std::memcpy(&info_o, data + idx + 22, 8);
                    std::cout << "    [OFFT @ 0x" << std::hex << idx << std::dec << "] Bitcode Offset: 0x" << std::hex << bc_o << " (Size: " << std::dec << bc_s << "B) | Info Table Offset: 0x" << std::hex << info_o << std::dec << "\n";
                }
                idx += 6 + olen;
        ...
        std::cout << "  --- 2. Function Information Table (LAYR, VATT, RFLT, SYMB, BITC, DEBG) ---\n";
        idx = tab_off;
        ...
        std::cout << "  --- 3. Bitcode Payload & Container Verifications ---\n";
        bool found_bc = false;
        for (size_t i = bc_off; i + 4 <= block_size; ++i) {
            if (data[i] == 0xBC && data[i+1] == 0xC0 && data[i+2] == 0xDE) {
                std::cout << "    [Success] Found valid LLVM Bitcode (BC\\xc0\\xde) at container offset 0x" << std::hex << i << " (Payload Size: " << std::dec << (block_size - i) << " bytes)\n";
                found_bc = true;
                break;
            }
        }
        if (!found_bc) {
            std::cout << "    [Raw Payload] Size: " << (block_size - bc_off) << " bytes\n";
        }
        return true;
    }
    ...
    static bool parseContainer(const std::string &filepath) {
        ...
        // Fat header is always big-endian, read first 4 bytes natively
        uint32_t magic = read_be32(data.data());

        if (magic == FAT_MAGIC_32 || magic == FAT_MAGIC_64) {
            uint32_t narch = read_be32(data.data() + 4);
            std::cout << "[FAT Universal Container] Magic: " << std::hex << magic << std::dec << " | Targets Found: " << narch << "\n";

            size_t arch_entry_size = 20;
            for (uint32_t i = 0; i < narch; ++i) {
                size_t offset = 8 + i * arch_entry_size;
                if (offset + arch_entry_size > file_size) break;

                uint32_t cputype = read_be32(data.data() + offset);
                uint32_t cpusubtype = read_be32(data.data() + offset + 4);
                uint32_t offset_val = read_be32(data.data() + offset + 8);
                uint32_t size_val = read_be32(data.data() + offset + 12);
                uint32_t align = read_be32(data.data() + offset + 16);

                std::cout << "\n----------------------------------------------------------------------\n";
                std::cout << " Target Architecture " << i << ": cputype=0x" << std::hex << cputype << " cpusubtype=0x" << cpusubtype
                          << " | Offset=0x" << offset_val << " Size=" << std::dec << size_val << "B Align=" << align << "\n";
                std::cout << "----------------------------------------------------------------------\n";

                if (offset_val + size_val <= file_size) {
                    parseMTLBBlock(data.data() + offset_val, size_val);
                } else {
                    std::cerr << "  Error: Slice offset out of bounds.\n";
                }
            }
        } else {
            // Raw MTLB Container
            parseMTLBBlock(data.data(), file_size);
        }
        return true;
    }
    ...
    static bool buildContainer(const std::string &bc_filepath, const std::string &output_filepath, const std::vector<std::string> &func_names) {
        ...
        for (size_t i = 0; i < func_names.size(); ++i) {
            ...
            dir_buf.insert(dir_buf.end(), reinterpret_cast<const uint8_t*>(&nlen), reinterpret_cast<const uint8_t*>(&nlen) + 2);
            ...
            dir_buf.insert(dir_buf.end(), reinterpret_cast<const uint8_t*>(&hlen), reinterpret_cast<const uint8_t*>(&hlen) + 2);
            dir_buf.insert(dir_buf.end(), hash.begin(), hash.end());

            uint16_t mlen = 8;
            uint64_t mdsz = bc_size + 1024;
            dir_buf.insert(dir_buf.end(), {'M', 'D', 'S', 'Z'});
            dir_buf.insert(dir_buf.end(), reinterpret_cast<const uint8_t*>(&mlen), reinterpret_cast<const uint8_t*>(&mlen) + 2);
            dir_buf.insert(dir_buf.end(), reinterpret_cast<const uint8_t*>(&mdsz), reinterpret_cast<const uint8_t*>(&mdsz) + 8);
            ...
            dir_buf.insert(dir_buf.end(), reinterpret_cast<const uint8_t*>(&olen), reinterpret_cast<const uint8_t*>(&olen) + 2);
            dir_buf.insert(dir_buf.end(), reinterpret_cast<const uint8_t*>(&bc_o), reinterpret_cast<const uint8_t*>(&bc_o) + 8);
            dir_buf.insert(dir_buf.end(), reinterpret_cast<const uint8_t*>(&bc_s), reinterpret_cast<const uint8_t*>(&bc_s) + 8);
            dir_buf.insert(dir_buf.end(), reinterpret_cast<const uint8_t*>(&info_o), reinterpret_cast<const uint8_t*>(&info_o) + 8);

            uint32_t zero32 = 0;
            dir_buf.insert(dir_buf.end(), {'E', 'N', 'D', 'T'});
            dir_buf.insert(dir_buf.end(), reinterpret_cast<const uint8_t*>(&zero32), reinterpret_cast<const uint8_t*>(&zero32) + 4);

            uint16_t tlen = 4;
            uint32_t layr_val = 0x00020600;
            tab_buf.insert(tab_buf.end(), {'L', 'A', 'Y', 'R'});
            tab_buf.insert(tab_buf.end(), reinterpret_cast<const uint8_t*>(&tlen), reinterpret_cast<const uint8_t*>(&tlen) + 2);
            tab_buf.insert(tab_buf.end(), reinterpret_cast<const uint8_t*>(&layr_val), reinterpret_cast<const uint8_t*>(&layr_val) + 4);

            uint32_t symb_val = 1;
            tab_buf.insert(tab_buf.end(), {'S', 'Y', 'M', 'B'});
            tab_buf.insert(tab_buf.end(), reinterpret_cast<const uint8_t*>(&tlen), reinterpret_cast<const uint8_t*>(&tlen) + 2);
            tab_buf.insert(tab_buf.end(), reinterpret_cast<const uint8_t*>(&symb_val), reinterpret_cast<const uint8_t*>(&symb_val) + 4);

            tab_buf.insert(tab_buf.end(), {'E', 'N', 'D', 'T'});
            tab_buf.insert(tab_buf.end(), reinterpret_cast<const uint8_t*>(&zero32), reinterpret_cast<const uint8_t*>(&zero32) + 4);
        }
...
int main(int argc, char **argv) {
    if (argc < 3) {
        std::cerr << "Usage: llvm-metallib parse <file.metallib>\n";
        std::cerr << "       llvm-metallib build <input.air/bc> <output.metallib> <func1> [func2...]\n";
        std::cerr << "       llvm-metallib lower <input.ll> <output.air>\n";
        return 1;
    }
