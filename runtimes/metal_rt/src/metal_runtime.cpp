...
// Memory Primitives (noexcept to match system header specifications)
...
// Target Min/Floor/Fract
double _target_mindd(double a, double b) { return a < b ? a : b; }
float _target_minff(float a, float b) { return a < b ? a : b; }
__fp16 _target_minDhDh(__fp16 a, __fp16 b) { return a < b ? a : b; }
...
double _target_floord(double x) { return my_floor_d(x); }
float _target_floorf(float x) { return my_floor(x); }
__fp16 _target_floorDh(__fp16 x) { return my_floor_h(x); }

__fp16 _target_fractDh(__fp16 x) { return x - my_floor_h(x); }
float _target_fast_fractf(float x) { return x - my_floor(x); }
