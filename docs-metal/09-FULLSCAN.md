...
コピーするジョブ」を `GITHUB_TOKEN` のまま動かしても同じ拒否を受ける。

解決は資格情報を替えること。`workflow` スコープを持つ PAT をリポジトリの
シークレットに入れ、同期ジョブがそれで checkout / push する。押すのは App
ではなくトークンの持ち主になるため制約を受けない。これを設置して同期が
成功することを確認済みで、`ci/workflows/` への変更は自動で
`.github/workflows/` へ反映される。

同期が入ったので人手は不要になったが、保護されたファイルの変更頻度は
低いに越したことがない。保護されているのはパスだけで振る舞いではないので、
